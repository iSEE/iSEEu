...
- Fix bug causing
  [`.setCachedCommonInfo()`](https://isee.github.io/iSEE/reference/setCachedCommonInfo.html)
  to cache `NULL` value for `valid.rowdata.names` information of
  `DynamicMarkerTable` panel instead of empty character vector.
...
- Added panel-specific tours for all panel classes via the
  [`.definePanelTour()`](https://isee.github.io/iSEE/reference/documentation-generics.html)
  generic.

## iSEEu 1.1.4

- Generalized the DE-related globals to work as patterns rather directly
  specifying the acceptable names.
- Align `DynamicMarkerTable`’s treatment of
...
- Renamed `DifferentialStatisticsTable` to the more appropriate
  ...
  `rowData`.
- Global parameters now only affect construction of `MAPlot`s and
  `VolcanoPlot`s.
...
- Replace `!=""` by [`nzchar()`](https://rdrr.io/r/base/nchar.html).
- Use 4-space indent.
