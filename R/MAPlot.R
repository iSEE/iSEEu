...
setMethod(".generateDotPlotData", "MAPlot", function(x, envir) {
    ...
    extra_cmds <- c(
        sprintf(".raw.p <- rowData(se)[[%s]];", deparse(x[["PValueField"]])),
        sprintf(
            ".de_status <- (p.adjust(.raw.p, method=%s) <= %s & abs(plot.data$Y) >= %s) * sign(plot.data$Y);",
...
#' @export
setMethod(".prioritizeDotPlotData", "MAPlot", function(x, envir) {
    cmds <- c(
...
#' @export
setMethod(".colorByNoneDotPlotScale", "MAPlot", function(x) { 
